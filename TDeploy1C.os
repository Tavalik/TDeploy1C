...
//	Запуск:
//		StartEnterprise[TestManager][WithProcessing[1|2|3|4|5]][ClearCache][NoWaitCompletion] 
//			- ЗапуститьПредприятие[МенеджерТестирования][СОбработкой[1|2|3|4|5]][ОчиститьКеш][НеЖдатьЗавершения]
...
Процедура Инициализация()
	...
	Если МассивФайловНастроек.Количество() = 0 Тогда
		...
		ПутьКФайлу = ОбъединитьПути(КаталогСценария, "TDeploy1C_Settings.json");
		Файл = Новый Файл(ПутьКФайлу);
		Если Не файл.Существует() Тогда
			Если СоздатьШаблонФайлаНастроек(ПутьКФайлу) Тогда
			...
			КонецЕсли;
		КонецЕсли;
		МассивФайловНастроек.Добавить(ПутьКФайлу);
	КонецЕсли;
	...
	#Область ФормированиеИтоговыхТаблиц
	...
	Пока Истина Цикл
		...
		Если файл.Существует() Тогда
			...
			Попытка
				...
				СтруктураДанных.Вставить("списокиб", ТаблицаБазДанных);
	...
	Для Каждого СтрокаБД Из ТаблицаБазДанных Цикл
		Для Каждого Колонка Из ТаблицаБазДанных.Колонки Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаБД[ИмяКолонки]) Тогда
				
				Для Каждого СтрокаОбщихНастроек Из ТаблицаОбщихНастроек Цикл
					СтрокаБД[ИмяКолонки] = СтрокаОбщихНастроек[ИмяКолонки];
				КонецЦикла;

				Если ИмяКолонки = "Синоним" Тогда
					СтрокаБД[ИмяКолонки] = СтрокаБД.Имя;
				КонецЕсли;
		...
		Для Каждого ТекущяИБ Из СтрокаСценария.СписокИБ Цикл
			...
			Для Каждого Скрипт Из СтрокаСценария.Сценарий Цикл
				...
				Если Лев(Скрипт, 15) = НРег("StartEnterprise")
					...
					Если Запуск1С.ЗапуститьПредприятие(ПутьКОбработке, ОжидатьЗавершения, ДопПараметрыЗапуска) Тогда
						Логирование.ЗаписатьСтрокуЛога("- УСПЕШНО", 3);
					Иначе
						Логирование.ЗаписатьСтрокуЛога("- ОШИБКА: " + Запуск1С.ТекстОшибки, 3);
						БылиОшибкиТекБД = Истина;
				...
				Если Скрипт = НРег("CheckChangesInRepository") 
					...
					Логирование.ЗаписатьСтрокуЛога("Ищем изменения в хранилище: ",2);
				...
				Если Лев(Скрипт, 29) = НРег("ConfigurationRepositoryCommit") 
					...
					Логирование.ЗаписатьСтрокуЛога("Выполняем помещение объектов в хранилище: ", 2);				
				...
				Если Скрипт = НРег("ConfigurationRepositoryBindCfg") 
					...
					Логирование.ЗаписатьСтрокуЛога("Подключаем конфигурацию к хранилищу: ", 2);				
				...
				Если Скрипт = НРег("GitPull")
					...
					Попытка
						ГитРепозиторий.УстановитьРабочийКаталог(СтрокаОбновления.GitЛокальныйРепозиторий);
						ГитИмяВетки = СтрокаОбновления.GitИмяВетки;
						Если Не ЗначениеЗаполнено(ГитИмяВетки) Тогда
							ГитИмяВетки = ГитРепозиторий.ПолучитьТекущуюВетку();
						КонецЕсли;
						ГитРепозиторий.Получить(СтрокаОбновления.GitУдаленныйРепозиторий, ГитИмяВетки);
						Логирование.ЗаписатьСтрокуЛога("- УСПЕШНО, имя ветки: " + ГитИмяВетки, 3);
					Исключение
						ОписаниеОшибки = ОписаниеОшибки();
						Логирование.ЗаписатьСтрокуЛога("- ОШИБКА:" + ОписаниеОшибки, 3);
						БылиОшибкиТекБД = Истина;
