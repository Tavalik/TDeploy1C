			...
			Для Каждого Скрипт Из СтрокаСценария.Сценарий Цикл
				...
				Если Скрипт = НРег("MergeCfg")
					...
					Если Не БылиОшибкиТекБД Тогда
						...
						ПолныйПутьКФайлуНастроек = ОбъединитьПути(ПутьДляФайловРаботыСХранилищем, "MergeSettings_" + ТекущяИБ + ".xml");
				...
				Если Скрипт = НРег("ConfigurationRepositoryLock") 
					...
					ПолныйПутьФайлаЗахвата = ОбъединитьПути(ПутьДляФайловРаботыСХранилищем, "RepositoryLock_" + ТекущяИБ + ".xml");
					...
					Если РабочийКаталог <> "" Тогда
						...
						Если Не БылиОшибкиТекБД Тогда
							...
							Иначе
								...
								Если Запуск1С.СохранитьКонфигурациюИзХранилищаВФайл(ПолноеИмяФайла) Тогда
									Логирование.ЗаписатьСтрокуЛога("- УСПЕШНО в " + ПолноеИмяФайла, 3);
								Иначе
									Логирование.ЗаписатьСтрокуЛога("- ОШИБКА: " + Запуск1С.ТекстОшибки, 3);
									БылиОшибкиТекБД = Истина;
				...
				Если Скрипт = НРег("CreateFileForRepositoryLockFromReport") 
					...
					ПолныйПутьОтчета = ОбъединитьПути(РабочийКаталог, "RepositoryReport_" + ТекущяИБ + ".txt");
					...
					ПолныйПутьРазрешенныхОбъектов = ОбъединитьПути(РабочийКаталог, "RepositoryList_" + ТекущяИБ + ".txt");
					...
					Если Не Файл.Существует() Тогда
						...
						КонецЕсли;						
					Иначе
						ПолныйПутьФайлаЗахвата = ОбъединитьПути(РабочийКаталог, "RepositoryLock_" + ТекущяИБ + ".xml");
					КонецЕсли;
					
					Если Не БылиОшибкиТекБД Тогда
						Логирование.ЗаписатьСтрокуЛога("Создаем файл для захвата объектов по истории хранилища: ", 2);
						ПолныйПутьКФайлуНеВошедшихОбъектов = ОбъединитьПути(РабочийКаталог, "RepositoryIgnoreList_" + ТекущяИБ + ".txt");				
						Файл = Новый Файл(ПолныйПутьКФайлуНеВошедшихОбъектов);
						Если Не Файл.Существует() Тогда
						...
						Если Запуск1С.СоздатьФайлДляЗахватаОбъектовИзФайлаИсторииХранилища(
							ПолныйПутьОтчета, ПолныйПутьФайлаЗахвата, 
							ПолныйПутьРазрешенныхОбъектов, ПолныйПутьКФайлуНеВошедшихОбъектов) Тогда
							Логирование.ЗаписатьСтрокуЛога("- УСПЕШНО", 3);
						Иначе
							Логирование.ЗаписатьСтрокуЛога("- ОШИБКА: " + Запуск1С.ТекстОшибки, 3);
							БылиОшибкиТекБД = Истина;
				...
				Если Скрипт = НРег("AllureGenerateReports") 
					...
					Если Не БылиОшибкиТекБД Тогда
						...
						СтрокаЗапуска = Команда.СформироватьСтрокуЗапускаПроцесса();
						Логирование.ЗаписатьСтрокуЛога("Строка запуска: " + СтрокаЗапуска, 2);
				...
				Если Лев(Скрипт, 7) = НРег("GitPull")
					Или Лев(Скрипт, 24) = НРег("ОбновитьИзГитРепозитория") Тогда
	
					НомерСкрипта = "";
					Для Сч = 1 По 5 Цикл
						Если Лев(Скрипт, 8) = НРег("GitPull" + Сч) 
							Или Лев(Скрипт, 25) = НРег("ОбновитьИзГитРепозитория" + Сч) Тогда
							НомерСкрипта = Строка(Сч);
							Прервать;
						КонецЕсли;
					КонецЦикла;
					...
					КонецЕсли;
					Команда.УстановитьРабочийКаталог(СтрокаОбновления["GitЛокальныйРепозиторий" + НомерСкрипта]);					
					Команда.ДобавитьПараметр("pull");
					Команда.ДобавитьПараметр(СтрокаОбновления["GitУдаленныйРепозиторий" + НомерСкрипта]);
					Если ЗначениеЗаполнено(СтрокаОбновления["GitИмяВетки" + НомерСкрипта]) Тогда
						Команда.ДобавитьПараметр(СтрокаОбновления["GitИмяВетки" + НомерСкрипта]);
					КонецЕсли;
					КодВозврата = Команда.Исполнить();
					Вывод = Команда.ПолучитьВывод();
